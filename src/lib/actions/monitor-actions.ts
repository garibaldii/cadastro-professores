...
/**
 * Atualiza um monitor existente (apenas SUPER_ADMIN)
 */
export async function updateMonitor(monitorId: string, formData: FormData) {
  ...
  if (!monitorId) return { ok:false, status:400, message:"ID do monitor é obrigatório." };
  ...
  try {
    const res = await fetch(`${API_URL}/monitores/${monitorId}`, {
      method:"PUT",
      headers: { "Content-Type":"application/json", Authorization:`Bearer ${token}` },
      cache:"no-store",
      body: JSON.stringify({ nome, email, tipo, nomePesquisaMonitoria, cargaHorariaSemanal, professorId, horarios }),
    ...
    if (!res.ok) {
      let msg = "Erro ao atualizar monitor.";
      try {
        const body = await res.json();
        msg = body?.mensagem || body?.message || msg;
      } catch {}
...
/**
 * Busca os detalhes de um monitor específico
 */
export async function buscarMonitor(monitorId: string) {
  const cookieStore = await cookies();
  const token = cookieStore.get("token")?.value;
  if (!token) return { ok:false, status:401, message:"Sessão expirada. Faça login novamente." };
  ...
  try {
    ...
    if (!res.ok) {
      let msg = "Erro ao buscar monitor.";
      try {
        const body = await res.json();
        msg = body?.mensagem || body?.message || msg;
      } catch {}
    ...
    const body = await res.json();
    const monitor = body.data || body;
    return { ok:true, status:200, monitor };
  } catch {
    return { ok:false, status:502, message:"Falha de comunicação com o servidor." };
  }
}
